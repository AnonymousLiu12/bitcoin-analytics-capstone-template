...
def load_data():
    """Load BTCUSDT daily data from CSV and normalize to expected schema."""
    # Try project root relative to this file
    base_dir = Path(__file__).parent.parent
    local_path = base_dir / "BTCUSDT_1d_2020_to_now.csv"

    # Fallback to CWD relative (for back-compat)
    if not local_path.exists():
        local_path = Path("BTCUSDT_1d_2020_to_now.csv")

    if not local_path.exists():
        raise FileNotFoundError(
            f"BTCUSDT data file not found at {local_path}. "
            "Please ensure BTCUSDT_1d_2020_to_now.csv exists at the project root."
        )

    logging.info(f"Loading BTCUSDT data from local file: {local_path}")
    df = pd.read_csv(local_path)

    # Set open_time as index
    if "open_time" not in df.columns:
        raise ValueError("open_time column not found in BTCUSDT data")
    ...
    # Use close as the daily price and map to existing expected column name.
    if "close" not in df.columns:
        raise ValueError("close column not found in BTCUSDT data")
    df["PriceUSD_coinmetrics"] = pd.to_numeric(df["close"], errors="coerce")
    ...
    logging.info(
        f"Loaded BTCUSDT data: {len(df)} rows, {df.index.min().date()} to {df.index.max().date()}"
    )
    return df
